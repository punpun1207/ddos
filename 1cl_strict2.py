...
- Per‑attacker rate limiting
- BLOCK ALL SOURCE IPs when attack detected (including ICMP)
- REST API only (no WebSocket inside)
"""
...
class SimpleMonitor13(SimpleSwitch13):
    def __init__(self, *args, **kwargs):
        ...
        # ML disabled – comment out to avoid errors
        # self.ml_model = None
        # self.scaler = None
        # try:
        #     import joblib
        #     if os.path.exists("model_5feat.pkl") and os.path.exists("scaler_5feat.pkl"):
        #         self.ml_model = joblib.load("model_5feat.pkl")
        #         self.scaler = joblib.load("scaler_5feat.pkl")
        #         self.logger.info("Loaded ML model with 5 features")
        #     else:
        #         self.logger.warning("ML model/scaler not found, using thresholds")
        # except Exception as e:
        #     self.logger.warning(f"ML load error: {e}, using thresholds")

        self._load_whitelist()
        hub.spawn(self._monitor_window)
        hub.spawn(self._cleanup_loop)
        hub.spawn(self._run_api)
        # hub.spawn(self._run_socketio)   # WebSocket disabled
    ...
    def _analyse_window(self):
        ...
        if icmp_ratio > thr_icmp_adapt:
            attack_type = 3
            confidence = calc_confidence(icmp_ratio, thr_icmp_adapt)
        elif udp_ratio > thr_udp_adapt:
            attack_type = 2
            confidence = calc_confidence(udp_ratio, thr_udp_adapt)
        elif syn_ratio > thr_syn_adapt:
            attack_type = 1
            confidence = calc_confidence(syn_ratio, thr_syn_adapt)
        elif ack_ratio > thr_ack_adapt:
            attack_type = 4
            confidence = calc_confidence(ack_ratio, thr_ack_adapt)
        else:
            attack_type = 0
            confidence = 0.0

        # Update baseline only when no attack
        if attack_type == 0:
        ...
        # Identify victim (most frequent dst IP)
        dst_freq = defaultdict(int)
        for ip in dst_list:
            dst_freq[ip] += 1
        victim = max(dst_freq, key=dst_freq.get) if dst_freq else "unknown"

        # ----- FIX: BLOCK ALL SRC IPs NOT IN WHITELIST -----
        src_freq = defaultdict(int)
        for ip in src_list:
            src_freq[ip] += 1
        attackers = [ip for ip in src_freq.keys() if ip not in self.whitelist]
        if not attackers and src_freq:
            attackers = list(src_freq.keys())[:1]   # fallback
        ...
        self.logger.warning(f"⚠️ {ATTACK_NAMES[attack_type]} | victim={victim} | conf={confidence:.2f} | attackers={len(attackers)}")
    ...
    def _rate_limit_attacker(self, attacker_ip, proto_num, rate_kbps=RATE_LIMIT_KBPS//2):
        ...
        self.logger.info(f"Rate-limited attacker {attacker_ip} @ {rate_kbps} kbps")
    ...
    def _drop_proto_to(self, victim, ip_proto):
        for dp in list(self.datapaths.values()):
            ...
            dp.send_msg(par.OFPFlowMod(datapath=dp, priority=200, match=match,
                                       ...
                                       command=ofp.OFPFC_ADD))
        self.logger.warning(f"Drop ip_proto={ip_proto} → {victim}")
    ...
    def _apply_rate_limit(self, victim, proto_num):
        for dp in list(self.datapaths.values()):
            ...
            dp.send_msg(par.OFPFlowMod(datapath=dp, priority=150, match=match, instructions=inst,
                                       hard_timeout=BLOCK_DURATION, command=ofp.OFPFC_ADD))
        self.logger.warning(f"Rate-limited proto={proto_num} → {victim} @ {RATE_LIMIT_KBPS}kbps")
